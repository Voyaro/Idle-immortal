    ...
    try:
        ...
        for player_id, player_data in data["players"].items():
            new_fields = {
                ...
                "boss_cooldowns": player_data.get("boss_cooldowns", {}),  # <--- TAMBAH INI
                "boss_kills": player_data.get("boss_kills", {}),          # <--- TAMBAH INI
                "inventory": player_data.get("inventory", {               # <--- UPDATE INI
                    "spirit_herb": 0, "spirit_water": 0, "spirit_crystal": 0,
                    "dragon_scale": 0, "phoenix_feather": 0, "dragon_heart": 0,
                    "void_crystal": 0, "primordial_essence": 0
                })
            }
            ...
            # Pastikan semua inventory items ada
            default_inventory = {
                "spirit_herb": 0, "spirit_water": 0, "spirit_crystal": 0,
                "dragon_scale": 0, "phoenix_feather": 0, "dragon_heart": 0,
                "void_crystal": 0, "primordial_essence": 0
            ...
            for item, default_count in default_inventory.items():
                if item not in player_data["inventory"]:
                    player_data["inventory"][item] = default_count
...
# ===============================
# Boss Dungeon System - BARU (5x REWARD + EXP)  <--- TARUH DI SINI
# ===============================
BOSS_DUNGEONS = {
    "ancient_dragon": {
        "name": "Ancient Dragon",
        ...
        "exp_reward": (2500, 5000),
        "spirit_stone_reward": (500, 1500),
        "cooldown": 86400,
        "description": "Ancient dragon guardian dengan damage tinggi",
        "special_drops": ["dragon_scale", "dragon_heart"],
        "max_party": 3
    },
    "phoenix_king": {
        "name": "Phoenix King", 
        ...
        "base_damage": 120,
        "reward_multiplier": 5.0,
        "exp_reward": (5000, 10000),
        "spirit_stone_reward": (1000, 2500),
        "cooldown": 172800,
        "description": "Raja phoenix dengan healing abilities",
        "special_drops": ["phoenix_feather", "spirit_crystal"],
        "max_party": 4
    },
    "void_titan": {
        "name": "Void Titan",
        ...
        "base_damage": 180,
        "reward_multiplier": 5.0,
        "exp_reward": (10000, 20000),
        "spirit_stone_reward": (1500, 4000),
        "cooldown": 259200,
        "description": "Titan dari void realm dengan attack massive",
        "special_drops": ["void_crystal", "primordial_essence"],
        "max_party": 5
    }
}
...
===================================
# Boss Dungeon Functions - BARU  <--- TARUH DI SINI
# ===============================
async def start_boss_battle(ctx, boss_key):
    # ... (seluruh fungsi start_boss_battle)

async def boss_battle_task(battle_id, ctx):
    # ... (seluruh fungsi boss_battle_task)

async def boss_battle_round(battle_id, ctx):
    # ... (seluruh fungsi boss_battle_round)

async def finish_boss_battle(battle_id, ctx):
    # ... (seluruh fungsi finish_boss_battle dengan update server stats)
...
async def cultivate_status(ctx):
    ...
    exp_gain = int(hours * 10000 * realm_data["exp_multiplier"])
...
# ===============================
# Command: boss - BARU  <--- TARUH DI SINI
# ===============================
@bot.command()
@commands.cooldown(1, 30, commands.BucketType.user)
async def boss(ctx, boss_name: str = None):
    # ... (seluruh fungsi boss command)

# ===============================
# Command: boss_stats - BARU
# ===============================
@bot.command()
async def boss_stats(ctx, member: discord.Member = None):
    # ... (seluruh fungsi boss_stats)
...
async def spirit_beasts(ctx):
    """Lihat semua spirit beast yang tersedia"""
    embed = discord.Embed(
        title="🐉 Spirit Beast Sanctuary",
        description="Tame spirit beasts untuk mendapatkan bonus permanen!",
        color=0x00ff00
    )
    ...
    for rarity, beasts in SPIRIT_BEASTS.items():
        beasts_text = ""
        for beast in beasts:
            beasts_text += f"{beast['emoji']} **{beast['name']}** - {beast['cost']} Spirit Stones\n"
            beasts_text += f"   Power: +{beast['power']} | Bonus: {', '.join([f'+{int(v*100)}% {k}' for k, v in beast['bonus'].items()])}\n"

        embed.add_field(
            name=f"{rarity.title()} Beasts",
            value=beasts_text or "No beasts available",
            inline=False
        )
...
async def tame(ctx, beast_name: str):
    ...
    if any(b["name"] == found_beast["name"] for b in p.get("spirit_beasts", [])):
        return await ctx.send("❌ Anda sudah menjinakkan spirit beast ini!")
    ...
    # Apply bonuses
    bonus_text = ""
    for bonus_type, bonus_value in found_beast["bonus"].items():
        if bonus_type == "all":
            p["base_power"] = int(p["base_power"] * (1 + bonus_value))
            bonus_text += f"+{int(bonus_value*100)}% semua stat\n"
        else:
            # Individual bonuses would be applied in specific calculations
            bonus_text += f"+{int(bonus_value*100)}% {bonus_type}\n"
    ...
    embed.add_field(name="Special Bonuses", value=bonus_text, inline=False)
...
async def set_beast(ctx, beast_name: str = None):
    ...
    if not beast_name:
        ...
        for i, beast in enumerate(p["spirit_beasts"]):
            ...
            embed.add_field(
                ...
                value=f"Power: +{beast['power']} | Bonus: {', '.join([f'+{int(v*100)}% {k}' for k, v in beast['bonus'].items()])}",
                inline=False
            )
    ...
    embed.add_field(name="Power Bonus", value=f"+{found_beast['power']}", inline=True)
    embed.add_field(name="Special Bonuses", value=", ".join([f"+{int(v*100)}% {k}" for k, v in found_beast["bonus"].items()]), inline=True)
